...
function charScore(data, pos) {
    var isWordStart = testWordStart(data, pos);
    var isPathTokenStart = data[pos - 1] === "/";
    var isFileName = pos > fileNameIndex;
    var score = 1;
...
function match(query, data, i, j, consequtiveMatch) {
    ...
    var score = query[i] === data[j] && query[i] == upQuery[i] ? 5 : 0;

    if (consequtiveMatch) {
        var headScore = charScore(data, j - consequtiveMatch) * 3 / 2 | 0;
        if (consequtiveMatch !== 1) {
            return headScore + score;
        }
    }
    return charScore(data, j) + score;
}
