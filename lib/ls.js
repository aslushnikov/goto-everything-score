...
function score(query, data, matchIndexes) {
    ...
    for(var i = 0; i < query.length; ++i) {
        ...
        for(var j = 0; j < data.length; ++j) {
            var skipCharScore = j === 0 ? 0 : scoreRow[j - 1];
            var prevCharScore = i === 0 || j === 0 ? 0 : score[i - 1][j - 1];
            var consequtiveMatch = i === 0 || j === 0 ? 0 : sequence[i - 1][j - 1];
            var pickCharScore = match(query, data, i, j, consequtiveMatch);
            if (prevCharScore + pickCharScore > skipCharScore && pickCharScore !== 0) {
                sequenceRow.push(consequtiveMatch + 1);
                scoreRow.push(prevCharScore + pickCharScore);
            } else {
                sequenceRow.push(0);
                scoreRow.push(skipCharScore);
            }
        }
    }
    if (matchIndexes) {
